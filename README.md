...
### 2. SignalR против WebSockets
Для реализации real-time синхронизации этапов воронки продаж и комментариев между клиентами выбран **ASP.NET Core SignalR** вместо "чистых" WebSockets по следующим архитектурным причинам:
*   **Транспортный Fallback:** SignalR автоматически переключается между транспортами (WebSockets -> Server-Sent Events -> Long Polling) в зависимости от возможностей сети клиента и настроек прокси-серверов.
*   **Встроенное управление группами:** Механизм хабов предоставляет готовые методы для организации каналов (например, объединение менеджеров внутри группы `SalesTeam`), избавляя от необходимости писать собственный брокер сообщений на WebSockets.
*   **Автоматическое переподключение и оркестрация соединений:** SignalR нативно управляет пингами (Keep-Alive), детекцией обрыва связи и повторной установкой соединения (Reconnection Policy) с обеих сторон.

### 3. Использование возможностей PostgreSQL для производительности
